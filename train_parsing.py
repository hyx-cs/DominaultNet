...
import util
import torchvision
from model.parsing import ParsingNet
...
trainset = DataLoader(traindata, batch_size=128, shuffle=True, num_workers=0)
valdata = dataset_parsingnet.Data(root=os.path.join(args.dir_data, args.data_val), args=args, train=False)
valset = DataLoader(valdata, batch_size=1, shuffle=False, num_workers=0)
...
for i in range(args.epochs):
    ...
    os.makedirs(os.path.join(args.save_path, args.writer_name1), exist_ok=True)
    os.makedirs(os.path.join(args.save_path, args.writer_name1, 'model'), exist_ok=True)
    torch.save(net.state_dict(),
               os.path.join(args.save_path, args.writer_name1, 'model', 'epoch{}.pth'.format(i + 1)))
    ...
    for batch, (lr, hr, filename) in enumerate(valset):
        lr, hr = util.prepare(lr), util.prepare(hr)
        sr = net(lr)
        psnr, _ = util.calc_metrics(hr[0].data.cpu(), sr[0].data.cpu())
        val_psnr_my = val_psnr_my + psnr
